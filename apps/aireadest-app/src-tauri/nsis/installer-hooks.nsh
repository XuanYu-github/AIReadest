...
!macro NSIS_HOOK_POSTINSTALL
    DetailPrint "Registering AIReadest Thumbnail Provider..."
    ...
    WriteRegStr HKCR "CLSID\${CLSID_READEST_THUMBNAIL}" "" "AIReadest Thumbnail Provider"
    WriteRegStr HKCR "CLSID\${CLSID_READEST_THUMBNAIL}\InprocServer32" "" "$INSTDIR\aireadest_thumbnail.dll"
    ...
    Delete "$DESKTOP\AIReadest.lnk"
    Delete "$SMPROGRAMS\AIReadest\AIReadest.lnk"
    RMDir "$SMPROGRAMS\AIReadest"
    ...
    CreateShortcut "$DESKTOP\AIReadest.lnk" "$INSTDIR\aireadest.exe"
    CreateDirectory "$SMPROGRAMS\AIReadest"
    CreateShortcut "$SMPROGRAMS\AIReadest\AIReadest.lnk" "$INSTDIR\aireadest.exe"
...
!macro NSIS_HOOK_PREUNINSTALL
    DetailPrint "Unregistering AIReadest Thumbnail Provider..."
    ...
    ; Delete the DLL file
    Delete "$INSTDIR\aireadest_thumbnail.dll"
