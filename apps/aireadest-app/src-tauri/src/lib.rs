...
fn set_windows_app_user_model_id() {
    ...
    let app_id = HSTRING::from("com.aireadest.app");
    if let Err(error) = unsafe { SetCurrentProcessExplicitAppUserModelID(&app_id) } {
        log::warn!("Failed to set Windows AppUserModelID: {error}");
    }
}
...
pub fn run() {
    ...
    #[cfg(target_os = "windows")]
    let builder = builder.manage(webview_buffer::CaptureWindowSourceState::default());
    ...
    let builder = builder
        .invoke_handler(tauri::generate_handler![
            ...
            #[cfg(target_os = "windows")]
            windows::disable_capture_window_transitions,
            #[cfg(target_os = "windows")]
            webview_buffer::set_capture_window_source_from_file,
            #[cfg(target_os = "windows")]
            webview_buffer::set_capture_window_source_from_monitor,
            #[cfg(target_os = "windows")]
            webview_buffer::post_capture_window_source_shared_buffer,
            #[cfg(target_os = "windows")]
            webview_buffer::clear_capture_window_source,
            #[cfg(target_os = "windows")]
            webview_buffer::take_capture_window_source_crop_png,
            #[cfg(target_os = "macos")]
        ...
        .setup(|#[allow(unused_variables)] app| {
            ...
            #[cfg(desktop)]
            {
                ...
                app.listen("window-ready", move |_| {
                    ...
                    webview
                        .eval("window.__AIREADEST_CLI_ACCESS = true; window.__OPENREADEST_CLI_ACCESS = true;")
            ...
            let eink_script = if is_eink {
                "window.__AIREADEST_IS_EINK = true; window.__OPENREADEST_IS_EINK = true;"
            } else {
                ""
            };
                    ...
                    builder = builder
                        ...
                        .icon(
                            tauri::image::Image::from_bytes(include_bytes!("../icons/icon.png"))
                                .expect("failed to load AIReadest window icon"),
                        )
                        .expect("failed to set AIReadest window icon");
                }
                #[cfg(target_os = "linux")]
                {
                    builder = builder
