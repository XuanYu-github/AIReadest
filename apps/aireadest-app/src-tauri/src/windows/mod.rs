...
fn hwnd_from_webview_window(window: &WebviewWindow) -> Result<HWND, String> {
    let handle = window
        .window_handle()
        .map_err(|error| format!("Failed to get native webview window handle: {error}"))?;
...
#[tauri::command]
pub fn disable_capture_window_transitions(app: AppHandle, label: String) -> Result<(), String> {
    let window = app
        .get_webview_window(&label)
        .ok_or_else(|| format!("Window not found: {label}"))?;
    let hwnd = hwnd_from_webview_window(&window)?;
    ...
    unsafe {
        ...
        .map_err(|error| format!("Failed to disable capture window transitions: {error}"))?;
    }
...
fn capture_screen_region_bgra(
    ...
    if width == 0 || height == 0 {
        return Err("Requested monitor region is empty".to_string());
    }

    let width_i32 = i32::try_from(width).map_err(|_| "Monitor width is too large".to_string())?;
    let height_i32 =
        i32::try_from(height).map_err(|_| "Monitor height is too large".to_string())?;
    ...
    if memory_dc.0.is_null() {
        ...
        return Err("Failed to create compatible screen device context".to_string());
    }
    ...
    if scanlines == 0 {
        return Err("Failed to extract screen capture pixels".to_string());
    }

    Ok((width, height, pixels))
}

pub(crate) fn capture_monitor_rgba(
    left: i32,
    top: i32,
    width: u32,
    height: u32,
) -> Result<(u32, u32, Vec<u8>), String> {
    let (width, height, mut bgra) = capture_screen_region_bgra(left, top, width, height)?;
    ...
    let (capture_backend, width, height, mut bgra) = match wgc_attempt {
        Ok((width, height, pixels)) => {
            ...
            if is_low_information_bgra(width, height, &pixels) {
                log::warn!("WGC capture looked low-information; returning it for frontend-side crop validation");
            }
...
pub fn take_cached_current_window_capture_png() -> Result<Option<CachedCapturePayload>, String> {
    ...
    Ok(cache.as_ref().map(|capture| CachedCapturePayload {
        ...
        png: encode_png_rgba(capture.width, capture.height, &capture.rgba).unwrap_or_default(),
    }))
}
