...
export class NativeAppService extends BaseAppService {
  ...
  override isEink = Boolean(window.__AIREADEST_IS_EINK || window.__OPENREADEST_IS_EINK);
  ...
  private legacyMigrationMarkerFilename = '.aireadest-legacy-migration-20260324';
  ...
  protected override async migrate20260324(): Promise<void> {
    const nextDataDir = await this.resolveFilePath('', 'Data');
    const rootDir = getDirPath(nextDataDir);
    const appDataParentDir = getDirPath(rootDir);
    const migrationMarker = await join(rootDir, this.legacyMigrationMarkerFilename);
    if (await this.fs.exists(migrationMarker, 'None')) {
      return;
    }

    const legacyCandidates = [
      { root: rootDir, data: await join(rootDir, LEGACY_DATA_SUBDIR) },
      { root: await join(appDataParentDir, 'com.bilingify.readest'), data: await join(await join(appDataParentDir, 'com.bilingify.readest'), LEGACY_DATA_SUBDIR) },
      { root: await join(appDataParentDir, 'AIReadest'), data: await join(await join(appDataParentDir, 'AIReadest'), LEGACY_DATA_SUBDIR) },
      { root: await join(appDataParentDir, 'Readest'), data: await join(await join(appDataParentDir, 'Readest'), LEGACY_DATA_SUBDIR) },
    ].filter((candidate, index, items) => candidate.data !== nextDataDir && items.findIndex((item) => item.data === candidate.data) === index);

    const nextSettingsPath = await join(rootDir, SETTINGS_FILENAME);
    const nextLibraryPath = await join(nextDataDir, 'Books', 'library.json');
    ...
    for (const candidate of legacyCandidates) {
      ...
      if (!bestLegacy || files.length > bestLegacy.files.length) {
        bestLegacy = { ...candidate, files };
      }
    }
    ...
    let migrated = false;
    if (!(await this.fs.exists(nextDataDir, 'None')) || !hasNextSettings || !hasNextLibrary) {
      await this.createDir(nextDataDir, 'None', true);
      await copyFiles(this, bestLegacy.data, nextDataDir);
      migrated = true;
    }

    if (bestLegacy.root !== rootDir) {
      for (const filename of [SETTINGS_FILENAME, `${SETTINGS_FILENAME}.bak`, '.persisted-scope', '.window-state.json']) {
        const source = await join(bestLegacy.root, filename);
        if (await this.fs.exists(source, 'None')) {
          await this.copyFile(source, await join(rootDir, filename), 'None').catch(() => undefined);
          migrated = true;
        }
      }
      const legacyWebdavDir = await join(bestLegacy.root, 'webdav');
      const nextWebdavDir = await join(rootDir, 'webdav');
      if (await this.fs.exists(legacyWebdavDir, 'None')) {
        await this.createDir(nextWebdavDir, 'None', true);
        await copyFiles(this, legacyWebdavDir, nextWebdavDir).catch(() => undefined);
        migrated = true;
      }
    }
