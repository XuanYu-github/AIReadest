...
export class WebAppService extends BaseAppService {
  ...
  protected override async migrate20260324(): Promise<void> {
    const db = await openIndexedDB();
    const files = await new Promise<Array<{ path: string; content: unknown }>>((resolve, reject) => {
      const request = db.transaction('files', 'readonly').objectStore('files').getAll();
      request.onerror = () => reject(request.error);
      request.onsuccess = () => resolve((request.result as Array<{ path: string; content: unknown }>) || []);
    });

    const legacyPrefix = `${LEGACY_DATA_SUBDIR}/`;
    const nextPrefix = `${DATA_SUBDIR}/`;
    const legacyFiles = files.filter((file) => file.path.startsWith(legacyPrefix));
    if (legacyFiles.length === 0) return;

    const hasNextSettings = files.some((file) => file.path === `${nextPrefix}settings.json`);
    const hasNextLibrary = files.some((file) => file.path === `${nextPrefix}Books/library.json`);
    if (hasNextSettings && hasNextLibrary) return;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      legacyFiles.forEach((file) => {
        const nextPath = `${nextPrefix}${file.path.slice(legacyPrefix.length)}`;
        store.put({ ...file, path: nextPath });
        store.delete(file.path);
      });
