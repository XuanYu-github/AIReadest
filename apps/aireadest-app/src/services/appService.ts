...
export abstract class BaseAppService implements AppService {
  ...
  protected CURRENT_MIGRATION_VERSION = 20260324;
  ...
  protected async runMigrations(lastMigrationVersion: number): Promise<void> {
    ...
    if (lastMigrationVersion < 20260324) {
      try {
        await this.migrate20260324();
      } catch (error) {
        console.error('Error migrating to version 20260324:', error);
      }
    }
  }
  ...
  protected async migrate20260324(): Promise<void> {
    // No-op by default. Platform-specific services can override.
  }
}
