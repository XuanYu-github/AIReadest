...
const BRAND_OVERRIDES: Record<string, string> = {
  'About AIReadest': 'About Readest',
  'Download AIReadest': 'Download Readest',
  'Exported from AIReadest': 'Exported from Readest',
};
...
const EXPLICIT_OVERRIDES: Record<string, Record<string, string>> = {
  en: {
    ...
    '赞助一下': 'Support AIReadest',
    '为我发"声"': 'Support AIReadest',
    '检查更新': 'Check for Updates',
    'AIReadest Support': 'AIReadest Support',
    'AIReadest Update': 'AIReadest Update',
    '请作者吃个鸡腿儿': 'Buy the maintainer a drumstick',
    '请作者吃顿拼好饭': 'Buy the maintainer a budget combo meal',
    'AIReadest 是 Readest 的独立分支，继续维护 EPUB、PDF、TXT 等阅读能力，并补强 WebDAV 同步与本地优先体验。':
      'AIReadest is an independent project evolved from Readest, focused on EPUB, PDF, TXT reading, stronger WebDAV sync, and a local-first library experience.',
    ...
    '当前页面先使用 AIReadest logo 占位，后续会切换为可远程替换的赞助二维码。':
      'This page is temporarily using the AIReadest logo as a placeholder. It will later switch to a remotely replaceable support QR code.',
    ...
    '保存到系统相册失败，请稍后重试。': 'Failed to save to the system photo album. Please try again later.',
    '长按约两秒即可自动保存到系统相册。': 'Long press for about two seconds to save it to your system photo album automatically.',
    '赞助海报暂未找到，请稍后重试。': 'The support poster is not available yet. Please try again later.',
    ...
    'AIReadest 的独立更新源正在接入中。当前阶段先保留独立“检查更新”页面，后续会接上远程版本信息、更新日志和下载分发。':
      'AIReadest is wiring up its independent update source. For now, this separate update page is kept in place and will later connect remote version info, changelogs, and downloads.',
    ...
    '这里会同步 AIReadest 的版本动向，也给你留好项目主页和最新版本入口。':
      'This page tracks AIReadest version updates and keeps the project homepage and latest release entry ready for you.',
    ...
    'AIReadest is an independent fork and continued re-development of Readest.':
    ...
    'Choose a new folder for AIReadest to move its data into.':
      'Choose a new folder for AIReadest to move its data into.',
    'AIReadest will use the selected Readest data after restart. Please restart the application to complete the switch.':
      'AIReadest will use the selected Readest data after restart. Please restart the application to complete the switch.',
    'This will switch AIReadest to the selected Readest data location. Make sure you selected the library you want to continue using.':
      'This will switch AIReadest to the selected Readest data location. Make sure you selected the library you want to continue using.',
  },
  'zh-CN': {
    License: '许可证',
    ...
    '请作者吃顿拼好饭': '请作者吃顿拼好饭',
    'AIReadest 是 Readest 的独立分支，继续维护 EPUB、PDF、TXT 等阅读能力，并补强 WebDAV 同步与本地优先体验。':
      'AIReadest 是基于 Readest 演进的独立项目，继续维护 EPUB、PDF、TXT 等阅读能力，并补强 WebDAV 同步与本地优先体验。',
    '如果这些改动帮你把书库稳定用起来了，可以扫下面这张码支持一下开发、测试设备和持续更新。':
      '如果这些改动帮你把书库稳定用起来了，可以扫下面这张码支持一下开发、测试设备和持续更新。',
    '长按保存赞助码': '长按保存赞助码',
    '保存后可在支付宝扫一扫里从相册识别，也可以直接点击下方按钮。':
    ...
    '请直接截图二维码，然后打开支付宝扫一扫从相册识别。': '请直接截图二维码，然后打开支付宝扫一扫从相册识别。',
    '请直接截图二维码。': '请直接截图二维码。',
    '打开支付宝扫一扫': '打开支付宝扫一扫',
    '再从相册里识别': '再从相册里识别',
    ...
    'Choose a new folder for AIReadest to move its data into.':
      '为 AIReadest 选择一个新的文件夹，用于迁移当前数据。',
    'AIReadest will use the selected Readest data after restart. Please restart the application to complete the switch.':
      'AIReadest 将在重启后使用所选的 Readest 数据。请重新启动应用以完成切换。',
    'This will switch AIReadest to the selected Readest data location. Make sure you selected the library you want to continue using.':
      '这将把 AIReadest 切换到所选的 Readest 数据位置。请确认你选择的是准备继续使用的书库。',
  },
  'zh-TW': {
    License: '授權條款',
    'Project Links': '專案連結',
    ...
    '请作者吃顿拼好饭': '請作者吃頓拼好飯',
    'AIReadest 是基于 Readest 演进的独立项目，继续维护 EPUB、PDF、TXT 等阅读能力，并补强 WebDAV 同步与本地优先体验。':
      'AIReadest 是基於 Readest 演進的獨立專案，持續維護 EPUB、PDF、TXT 等閱讀能力，並補強 WebDAV 同步與本地優先體驗。',
    '如果这些改动帮你把书库稳定用起来了，可以扫下面这张码支持一下开发、测试设备和持续更新。':
    ...
    '收款码已就绪': '收款碼已就緒',
    '当前页面先使用 AIReadest logo 占位，后续会切换为可远程替换的赞助二维码。':
      '目前頁面先使用 AIReadest logo 作為佔位，後續會切換為可遠端替換的贊助 QR 碼。',
    'Android 端可以继续长按图片保存；桌面端会显示显式按钮，避免把长按当成主要操作。':
    ...
    '请直接截图二维码，然后打开支付宝扫一扫从相册识别。': '請直接截圖二維碼，然後打開支付寶掃一掃從相簿識別。',
    '请直接截图二维码。': '請直接截圖二維碼。',
    '打开支付宝扫一扫': '打開支付寶掃一掃',
    '再从相册里识别': '再從相簿裡識別',
    ...
    'AIReadest 的独立更新源正在接入中。当前阶段先保留独立“检查更新”页面，后续会接上远程版本信息、更新日志和下载分发。':
      'AIReadest 的獨立更新來源正在接入中。現階段先保留獨立「檢查更新」頁面，後續會接上遠端版本資訊、更新日誌與下載分發。',
    '现在你可以先通过项目主页查看最新进展；Android 包会继续按 ARM64 优先提供。':
      '現在你可以先透過專案首頁查看最新進展；Android 套件會繼續以 ARM64 為優先。',
    '这里会同步 AIReadest 的版本动向，也给你留好项目主页和最新版本入口。':
      '這裡會同步 AIReadest 的版本動向，也把專案首頁與最新版本入口留給你。',
    '如果一时打不开或加载不出来，多半是 GitHub 网络波动，换个时间或者稍后再试就好。':
    ...
    'Need help? Contact our support team at support@readest.com':
      '需要協助？請前往 AIReadest 倉庫提交 issue。',
    'Choose a new folder for AIReadest to move its data into.':
      '為 AIReadest 選擇一個新的資料夾，用來遷移目前資料。',
    'AIReadest will use the selected Readest data after restart. Please restart the application to complete the switch.':
      'AIReadest 將在重新啟動後使用所選的 Readest 資料。請重新啟動應用程式以完成切換。',
    'This will switch AIReadest to the selected Readest data location. Make sure you selected the library you want to continue using.':
      '這將把 AIReadest 切換到所選的 Readest 資料位置。請確認你選擇的是準備繼續使用的書庫。',
  },
};
...
export const applyAIReadestTranslationOverrides = (instance: typeof i18n, lng: string) => {
  ...
  for (const [targetKey, sourceKey] of Object.entries(BRAND_OVERRIDES)) {
    ...
    if (typeof sourceValue === 'string' && sourceValue.trim()) {
      overrides[targetKey] = sourceValue.replaceAll('Readest', 'AIReadest');
    }
  }
