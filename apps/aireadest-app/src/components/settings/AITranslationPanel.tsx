...
const UNSUPPORTED_LEGACY_PROTOCOL_PATTERN =
  /unsupported legacy protocol|\/v1\/chat\/completions.*not supported|please use\s*\/v1\/responses/i;
...
const AITranslationPanel: React.FC<AITranslationPanelProps> = () => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const useResponsesApi = /^gpt-5/i.test(model.trim());

      const buildTauriPayload = (responsesApi: boolean) =>
        responsesApi
          ? {
              model,
              input: [
                {
                  role: 'system',
                  content: [{ type: 'input_text', text: 'You are a translator.' }],
                },
                {
                  role: 'user',
                  content: [{ type: 'input_text', text: 'Translate "hello" to Chinese.' }],
                },
              ],
      ...
      if (isTauriAppPlatform()) {
        const sendTauriRequest = (responsesApi: boolean) =>
          tauriFetch(`${normalizedBaseUrl}${responsesApi ? '/responses' : '/chat/completions'}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${apiKey}`,
            },
            body: JSON.stringify(buildTauriPayload(responsesApi)),
          });

        response = await sendTauriRequest(useResponsesApi);

        if (!response.ok && !useResponsesApi) {
          preReadErrorText = await response.text().catch(() => '');
          if (UNSUPPORTED_LEGACY_PROTOCOL_PATTERN.test(preReadErrorText)) {
            response = await sendTauriRequest(true);
            preReadErrorText = '';
          }
        }
      } else {
      ...
      if (response.ok) {
        ...
        const errorText = preReadErrorText || (await response.text().catch(() => response.statusText));
        eventDispatcher.dispatch('toast', {
