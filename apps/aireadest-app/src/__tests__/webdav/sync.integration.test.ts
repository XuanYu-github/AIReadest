// Copyright (c) 2026 XuanYu-github
...
describe('WebDAV sync integration', () => {
  ...
  it('uploads local files to remote when remote missing', async () => {
    ...
    expect(remotePaths).toContain('/dav/Books/library.json');
    expect(remotePaths).toContain('/dav/Books/hash1/The Alchemist.epub');
    expect(remotePaths).toContain('/dav/Books/hash1/config.json');
    expect(remotePaths).toContain('/dav/System/webdav-sync-state.json');
    expect(stores.Settings.has('webdav/AIReadest/webdav-sync-state.json')).toBe(true);
  });
  ...
  it('downloads remote files to local when local missing', async () => {
    ...
    await fetch(`${webdav.serverUrl}/dav/Books/library.json`, {
      method: 'PUT',
      headers: { Authorization: auth, 'Content-Type': 'application/json' },
      body: JSON.stringify([book]),
    });
    await fetch(`${webdav.serverUrl}/dav/Books/hash2/Dune.epub`, {
      method: 'PUT',
      headers: { Authorization: auth },
      body: new Uint8Array([9, 9, 9]),
    });
    await fetch(`${webdav.serverUrl}/dav/Books/hash2/config.json`, {
      method: 'PUT',
      headers: { Authorization: auth, 'Content-Type': 'application/json' },
  ...
  it('detects conflicts when both sides changed and strategy is manual', async () => {
    ...
    const auth = `Basic ${Buffer.from('u:p', 'utf-8').toString('base64')}`;
    await fetch(`${webdav.serverUrl}/dav/Books/hash3/config.json`, {
      method: 'PUT',
      headers: { Authorization: auth, 'Content-Type': 'application/json' },
