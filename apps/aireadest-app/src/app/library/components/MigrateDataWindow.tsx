...
import { DATA_SUBDIR, LEGACY_DATA_SUBDIR, SETTINGS_FILENAME } from '@/services/constants';
...
export const MigrateDataWindow = () => {
  ...
  const findExistingDataMatch = async (selectedDir: string): Promise<ExistingDataMatch | null> => {
    ...
    for (const candidateDirName of [DATA_SUBDIR, LEGACY_DATA_SUBDIR]) {
      const nestedDataDir = await join(selectedDir, candidateDirName);
      ...
      if (selectedDir) {
        if (mode === 'connect') {
          ...
          if (!existingDataMatch) {
            throw new Error(
              _('No compatible AIReadest or Readest data was found in the selected folder.'),
            );
          }
  ...
  const handleSelectedNewDir = async (
    ...
    try {
      if (mode === 'connect') {
        ...
        if (!existingDataMatch) {
          throw new Error(_('No compatible AIReadest or Readest data was found in the selected folder.'));
        }
        setNewDataDir(existingDataMatch.dataDir);
      } else {
  ...
  const selectionTitle = isConnectMode ? _('Existing AIReadest or Readest Data') : _('New Data Location');
  ...
  const selectionHint = isConnectMode
    ? _('Select the folder that already contains AIReadest or Readest data, or its parent folder.')
    : _('Choose a new folder for AIReadest to move its data into.');
          ...
          <div className='space-y-3'>
            <div className='bg-base-200 inline-flex rounded-xl p-1'>
              ...
              <button
                ...
                disabled={isMigrating}
              >
                {_('Use Existing AIReadest or Readest Data')}
              </button>
            </div>
            ...
            <div className='space-y-3'>
              ...
              <div className='bg-success/10 border-success/20 rounded-lg border p-3'>
                <p className='text-success/80 text-sm'>
                  {isConnectMode
                    ? _(
                        'AIReadest will use the selected AIReadest or Readest data after restart. Please restart the application to complete the switch.',
                      )
                    : _(
              ...
              <div className='flex items-start gap-2'>
                ...
                <div className='space-y-1'>
                  ...
                  <p className='text-base-content/80 text-sm'>
                    {isConnectMode
                      ? _(
                          'This will switch AIReadest to the selected AIReadest or Readest data location. Make sure you selected the library you want to continue using.',
                        )
                      : _(
