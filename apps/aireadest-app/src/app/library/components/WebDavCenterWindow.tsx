...
const formatWebDavConnectionError = (
  ...
  if (trimmedError) {
    return `${t('连接失败')}: ${t(trimmedError)}${debug ? ` | ${debug}` : ''}`;
  }

  if (status === 401) {
    return `${t('连接失败')}: ${t('用户名或密码错误')}${debug ? ` | ${debug}` : ''}`;
  }
  if (status === 403) {
    return `${t('连接失败')}: ${t('服务器拒绝访问该路径')}${debug ? ` | ${debug}` : ''}`;
  }
  if (status === 404) {
    return `${t('连接失败')}: ${t('服务器地址或远端路径不存在')}${debug ? ` | ${debug}` : ''}`;
  }
  if (status === 405) {
    return `${t('连接失败')}: ${t('服务器不支持 WebDAV PROPFIND 请求')}${debug ? ` | ${debug}` : ''}`;
  }
  if (status === 301 || status === 302 || status === 307 || status === 308) {
    return `${t('连接失败')}: ${t('服务器发生重定向，请检查地址是否正确')}${debug ? ` | ${debug}` : ''}`;
  }
  if (status && status >= 500) {
    return `${t('连接失败')}: ${t('服务器内部错误')} (${status})${debug ? ` | ${debug}` : ''}`;
  }
  if (status && status > 0) {
    return `${t('连接失败')}: HTTP ${status}${debug ? ` | ${debug}` : ''}`;
  }

  return `${t('连接失败')}${debug ? ` | ${debug}` : ''}`;
};
...
export const WebDavCenterWindow = () => {
  ...
  const testConnection = async () => {
    try {
      ...
      if (!result.ok) {
        showToast(formatWebDavConnectionError(_, result.status, result.error, result.debug), 'error');
        return;
      }
      if (result.resolvedRootPath && result.resolvedRootPath !== profile.remotePath) {
        setEditing((prev) => ({ ...prev, remotePath: result.resolvedRootPath! }));
        showToast(`${_('连接成功')}，${_('已自动修正远端路径')}：${result.resolvedRootPath}`, 'success');
      } else {
        showToast(_('连接成功'), 'success');
      }
      await loadRemoteBooks(client);
    } catch (error) {
      showToast(formatWebDavConnectionError(_, 0, (error as Error).message), 'error');
    }
  };
