...
import { LEGACY_DATA_SUBDIR, SETTINGS_FILENAME } from '@/services/constants';
...
const LEGACY_DATA_PROMPT_SESSION_KEY = 'aireadest:legacy-data-prompted';
...
const LibraryPageContent = ({ searchParams }: { searchParams: ReadonlyURLSearchParams | null }) => {
  ...
  const findLegacyReadestDataDir = useCallback(async () => {
    ...
    const candidates = [
      await joinPaths(currentRootDir, LEGACY_DATA_SUBDIR),
      await joinPaths(await joinPaths(appDataParentDir, 'AIReadest'), LEGACY_DATA_SUBDIR),
      await joinPaths(await joinPaths(appDataParentDir, LEGACY_DATA_SUBDIR), LEGACY_DATA_SUBDIR),
    ].filter((candidate, index, items) => candidate !== currentDataDir && items.indexOf(candidate) === index);
    ...
    for (const legacyDataDir of candidates) {
      ...
      if (hasLegacyLibrary || hasLegacySettings) {
