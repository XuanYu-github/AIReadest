# Windows Thumbnail Provider for AIReadest

This crate provides Windows Explorer thumbnail support for eBook files when AIReadest is set as the default application.

## Features

- **Automatic Cover Extraction**: Extracts cover images from EPUB, MOBI, AZW, AZW3, FB2, CBZ, CBR files
- **AIReadest Branding**: Adds a small AIReadest icon overlay at the bottom-right corner
- **Smart Caching**: Caches generated thumbnails for faster subsequent loads
- **File Association Aware**: Only shows thumbnails when AIReadest is the default app for the file type
...
The thumbnail provider DLL is automatically registered when AIReadest is installed via the NSIS installer.
...
For local development and testing, build the Windows DLL (or the library) from the AIReadest Tauri app folder and register it manually. The legacy CLI test harness used to live in the separate `packages/tauri` workspace, but the thumbnail handler implementation now lives inside AIReadest's Tauri app.

Build the DLL (for Windows explorer integration):

```powershell
cd apps/aireadest-app/src-tauri
...
This generates a thumbnail with the AIReadest overlay at the specified size.
...
│                          │    (is AIReadest the default?)         │
│                          │         │                             │
│                          │         ▼ (if yes)                    │
│                          │    Extract Cover Image                │
│                          │         │                             │
│                          │         ▼                             │
│                          │    Add AIReadest Overlay               │
│                          │         │                             │
│                          │         ▼                             │
│                          │    Return HBITMAP                     │
│                          │                                       │
...
3. It checks if `aireadest.exe` is the default application for that file type using `AssocQueryStringW`
4. If AIReadest is the default, it extracts the cover and generates the thumbnail
5. If AIReadest is NOT the default, it returns `S_FALSE` to let Windows use other handlers

This ensures thumbnails only appear for files the user has associated with AIReadest.
